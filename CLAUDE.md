...
- No debt-fund/property capital gains, no foreign income/assets, no business income.

**CA-prepared ITR-2** (`apps/web/src/pages/ca/filing/CAITR2Filing.jsx`, `/ca/clients/:clientId/itr2`): a direct parallel of the CA-prepared ITR-1 flow (`CAITRFiling.jsx`) — same client pre-fill, draft-save-per-step, `ca_admin`-only final submit, and "Send for Client Approval" success action — but with the self-service wizard's 5 steps (incl. house property `Form.List` + capital gains fields) and `compareRegimesWithCG`. Backend: `saveDraftITR2ForClient`/`submitITR2ForClient` in `filing.service.js`/`filing.controller.js`, routed at `POST /ca/clients/:clientId/draft/itr2` and `/submit/itr2` (`ca.routes.js`, `requireFeature("ITR_2")`-gated like every other ITR-2 route). `ClientWorkspace.jsx` offers an ITR-1/ITR-2 choice (`Dropdown`) when a client has no filing yet, and routes "Continue/Edit/Revise Filing" by the existing filing's `itrType`.

**itrType-aware downstream fixes required for this to work end-to-end** — three places previously assumed every `Filing` was ITR-1 and would have silently shown blank/wrong data or dead-ended for an ITR-2 filing:
- `ca-client.service.js`'s `getClient()` `.select()` projection now also includes `itr2Data.*` fields (previously only projected `itr1Data.*`, so `itr2Data` was silently omitted from `ClientWorkspace.jsx`'s `filings[]`).
- `approval.service.js`'s `sendApprovalRequest`/`getApprovalSummary` and `ApprovePage.jsx` branch on `filing.itrType` to read `itr2Data` instead of `itr1Data`, and the approval summary gains a "House Property & Capital Gains" section (`summary.houseProperties`/`housePropertyNetIncome`/`capitalGains`) shown only for ITR-2.
- `EFilingPage.jsx` — previously hardcoded to `itr1Data`/`itrType === "ITR-1"` throughout (auto-pick filter, PAN lookups, EVC calls, XML filename, copy strings), which blocked **both** self-service and CA-prepared ITR-2 from ever reaching e-filing. Now reads via a `getFilingData(filing)` helper that picks `itr1Data`/`itr2Data` by `filing.itrType`.
...
15. ~~Platform Admin: CA firm management~~ — Done. `apps/api/src/modules/admin/admin-firms.*` (routes mounted under `/admin/firms`, `requireAdmin`-gated like every other admin route) and `apps/web/src/pages/admin/AdminFirms.jsx` (new "CA Firms" tab in `AdminLayout.jsx`, between Users and Feature Flags). Search by firm name/ICAI number; each row shows live-computed `clientCount`/`filingCount`/`memberCount`. **Stats are computed against the firm admin's `adminUserId`, not the firm's own `_id`** — `CAClient`/`Filing` key off the resolved CA-admin userId (see `resolveOwnerUserId` in "Role & Permission Model"), not `caFirmId`, so `getFirmStats()` in `admin-firms.service.js` queries `CAClient.countDocuments({ caId: adminUserId })` / `Filing.countDocuments({ userId: adminUserId, caClientId: { $ne: null } })` rather than anything keyed by the firm document. Deactivating a firm (`PATCH /admin/firms/:id/active`) sets `CAFirm.isActive: false` **and cascades** `User.updateMany({ caFirmId }, { isActive: false })`, blocking every `ca_admin`/`ca_staff`/`ca_readonly` on that firm from logging in (reuses the existing `isActive` login gate already enforced in `auth.service.js` — no new auth check needed). Reactivating a firm restores the whole team symmetrically. Both actions log to the existing `AuditLog` model via two new enum values, `FIRM_ACTIVATED`/`FIRM_DEACTIVATED`.
...
19. **Per-firm configurable email/SMS providers (BYO communications)** — Today every CA firm's approval/invite emails go through the platform's single Gmail account and Twilio number, hardcoded in `apps/api/.env`. Mirror the existing per-firm ITD credential pattern (`CAFirm.itdApiBaseUrl`/`itdApiKeyEncrypted`, resolved firm-key-first then platform-`.env`-fallback) for email and SMS too: let a CA firm optionally configure its own SMTP server or SMS gateway from CA Dashboard → Settings, instead of being locked into the platform's. v1 provider list: generic SMTP (covers Office365/Zoho/SES/SendGrid-via-SMTP without per-vendor code) + one DLT-relevant SMS gateway (e.g. MSG91) alongside the existing platform Gmail/Twilio defaults. Scoped to `CAFirm` only — individual taxpayers never send notifications to anyone, so there's no per-taxpayer config to add.
